...
export class Friendship extends Service {
 ...
 async patch (id: Id, data, params) {
    ...
    const friendship = await this.get(id, params);

    if(!friendship)
        throw new NotFound('Friendship records not found');

    if(friendship.targetId !== params.User.id)
        throw new BadRequest('You are already friends or you are not authorized to modify this record');
