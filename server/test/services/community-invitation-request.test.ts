...
describe("'communityInvitationRequest' service", () => {
  ...
  beforeAll(async () => {
    ...
    // const Roles = app.get('sequelizeClient').models.CommunityRole;
    // // creating the roles : 
    // const responses = await Promise.all(
    //   roles.map(({ name, role_access_level }) =>
    //     server.post(rolesEndpoint).send({
    //       name,
    //       role_access_level
    //     })))

    // server = request(app);
    // roles = responses.map((response) => response.body);
    ...
    //   //   getRandUsers(5).map((user) =>
    //   //     server.post(userEndpoint).send({ ...user, id: undefined })))
    //   const u = await server.post(userEndpoint).send({})
    //   console.log(u.body)
    ...
    // invites = sendInvitation(server, endpoint);
  }, 100000);
  ...
  //   const invitationsISent = await server
  ...
  //   const newInvitation = await server
